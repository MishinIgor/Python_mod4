...
def file_show(request, id):
    dir = listdir('APP/static/files')
    if int(id) > len(dir):
        return HttpResponseForbidden()
    elif int(id) == 0:
        return HttpResponseForbidden()
    else:
        return FileResponse(open(f'APP/static/files/{id}.png','rb'))
    
def download_file(request):
    #Путь к файлу, который вы хотщите отправить
    file_path = 'APP/static/files/file.txt'
    ...
    if os.path.exists(file_path):
        ...
        HttpResponseNotFound('File not found, sorry...i cry everyday...')
        
def download_image(request):
    #Путь к изображению
    image_path = '/APP/static/files/1.png'
